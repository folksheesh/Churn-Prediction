...
export default function Home() {
  ...
  const [uploadStatus, setUploadStatus] = useState<{
    ...
    message: string;
    errors: string[];
    summary?: { total: number; high_risk: number; medium_risk: number; low_risk: number; avg_churn_probability: number };
    results?: { name: string; churn_probability: number | null; risk_level: string; region: string; plan_tier: string; age: number | null }[];
  } | null>(null);
  const [uploading, setUploading] = useState(false);
  const [uploadHistory, setUploadHistory] = useState<{count: number, date: string}[]>([]);
  ...
  const fetchUploadHistory = async () => {
    try {
      const res = await axios.get(`${API_BASE}/analytics/activity-logs`, { params: { limit: 50 } });
      const importLogs = res.data.filter((log: any) => log.action === "CSV Import");
      const history = importLogs.map((log: any) => {
      ...
      <main className="flex-1 flex flex-col min-w-0 max-w-[1280px] mx-auto px-8 py-6">
        ...
        {activeTab === "dashboard" && (
          <div className="space-y-8 animate-fadeIn">
            ...
            {/* BOTTOM SECTION: REGION RETENTION */}
            ...
            <div className="md:col-span-2 space-y-6">
              ...
              {uploadStatus?.success === false ? (
                <div className="space-y-6 animate-fadeIn">
                    ...
                    <div className="glass-card rounded-2xl p-6">
                      ...
                      <div className="space-y-2">
                        {uploadStatus.errors.map((err, idx) => (
                          <div key={idx} className="flex items-start gap-3 bg-rose-50/60 border border-rose-100 rounded-xl px-4 py-3">
                  ...
                  <div className="bg-blue-50/50 border border-blue-100 rounded-2xl p-6">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <h4 className="text-sm font-bold text-slate-900">Field Requirements</h4>
                    </div>
                    <ul className="space-y-1.5 text-xs text-slate-600">
                      <li className="flex items-start gap-2"><span className="text-blue-400 mt-0.5">•</span><span><span className="font-semibold text-slate-700">age</span> — Integer, 0–120</span></li>
                      <li className="flex items-start gap-2"><span className="text-blue-400 mt-0.5">•</span><span><span className="font-semibold text-slate-700">days_since_joined</span> (Customer Tenure) — Integer, 0–3650 days (0–120 months)</span></li>
                      <li className="flex items-start gap-2"><span className="text-blue-400 mt-0.5">•</span><span><span className="font-semibold text-slate-700">avg_transaction_value</span> (Monthly Subscription Value) — Non-negative number</span></li>
                      <li className="flex items-start gap-2"><span className="text-blue-400 mt-0.5">•</span><span><span className="font-semibold text-slate-700">logins_90d, api_calls_90d, tickets_opened_90d, days_since_active</span> — Non-negative integers</span></li>
                    </ul>
                  </div>
                ...
                <form onSubmit={handleFileUpload} className="glass-card rounded-2xl p-8 flex flex-col items-center justify-center min-h-[300px] border-2 border-dashed border-slate-200 hover:border-brand-400 bg-slate-50/50 hover:bg-white transition-all cursor-pointer relative group animate-fadeIn">
                  ...
                  {uploadStatus?.success && (
                    <div className="absolute inset-0 bg-white rounded-2xl flex flex-col items-center justify-center z-30 animate-fadeIn border border-emerald-200">
                      <CheckCircle2 className="w-10 h-10 text-emerald-500 mb-3" />
                      <h3 className="text-base font-bold text-slate-900 mb-1">Upload Successful</h3>
                      <p className="text-xs text-slate-500 mb-5">{uploadStatus.message}</p>
                      <button
                        type="button"
                        onClick={() => { setUploadFile(null); setUploadStatus(null); }}
                        className="px-6 py-2.5 bg-emerald-500 hover:bg-emerald-600 text-white font-bold rounded-xl transition-colors cursor-pointer text-sm"
                      >
                        Upload Another
                      </button>
                    </div>
              ...
              {uploadStatus?.success && uploadStatus.results && uploadStatus.results.length > 0 && (
                <div className="space-y-6 animate-fadeIn mt-6">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    {[
                      { label: "Total Customers", value: uploadStatus.summary?.total ?? 0, color: "text-slate-800", bg: "bg-slate-50 border-slate-200" },
                      { label: "High Risk",       value: uploadStatus.summary?.high_risk ?? 0,   color: "text-rose-600",    bg: "bg-rose-50 border-rose-200" },
                      { label: "Medium Risk",     value: uploadStatus.summary?.medium_risk ?? 0, color: "text-amber-600",   bg: "bg-amber-50 border-amber-200" },
                      { label: "Low Risk",        value: uploadStatus.summary?.low_risk ?? 0,    color: "text-emerald-600", bg: "bg-emerald-50 border-emerald-200" },
                    ].map((s, i) => (
                  ...
                  <div className="bg-blue-50 border border-blue-100 rounded-2xl p-4 flex items-center gap-4">
                    ...
                    <div>
                      <p className="text-xs font-semibold text-slate-500">Average Churn Probability</p>
                      <p className="text-2xl font-black text-slate-900 font-outfit">{uploadStatus.summary?.avg_churn_probability ?? 0}%</p>
                    </div>
                    <div className="ml-auto">
                      <button
                        onClick={() => { setUploadFile(null); setUploadStatus(null); }}
                        className="px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white text-xs font-bold rounded-xl transition-colors"
                      >
                        Upload Another
                      </button>
                    </div>
                  ...
                  <div className="glass-card rounded-2xl overflow-hidden">
                    <div className="p-5 border-b border-slate-100 flex items-center justify-between">
                      <h4 className="text-sm font-extrabold text-slate-900">Prediction Results</h4>
                      <span className="text-xs text-slate-400 font-medium">{uploadStatus.results.length} customer(s)</span>
                    </div>
                    <div className="overflow-x-auto">
                      <table className="w-full text-sm">
                        <thead className="bg-slate-50 border-b border-slate-100">
                          <tr>
                            ...
                            <th className="text-left px-5 py-3 text-xs font-bold text-slate-500 uppercase tracking-wide">Customer</th>
                            <th className="text-left px-5 py-3 text-xs font-bold text-slate-500 uppercase tracking-wide">Age</th>
                            <th className="text-left px-5 py-3 text-xs font-bold text-slate-500 uppercase tracking-wide">Region</th>
                            <th className="text-left px-5 py-3 text-xs font-bold text-slate-500 uppercase tracking-wide">Plan</th>
                            <th className="text-left px-5 py-3 text-xs font-bold text-slate-500 uppercase tracking-wide">Churn Probability</th>
                            <th className="text-left px-5 py-3 text-xs font-bold text-slate-500 uppercase tracking-wide">Risk Level</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100">
                              ...
                              <tr key={idx} className="hover:bg-slate-50 transition-colors">
                                ...
                                <td className="px-5 py-3.5">
                                  <div className="flex items-center gap-2.5">
                                    <div className="w-7 h-7 rounded-full bg-brand-100 text-brand-700 flex items-center justify-center text-[10px] font-black shrink-0">
                                ...
                                <td className="px-5 py-3.5 text-xs text-slate-600">{row.age ?? '-'}</td>
                                <td className="px-5 py-3.5 text-xs text-slate-600">{row.region}</td>
                                <td className="px-5 py-3.5 text-xs text-slate-600">{row.plan_tier}</td>
                                <td className="px-5 py-3.5">
              ...
              <div className="glass-card rounded-2xl p-6 mt-6">
                ...
                {uploadHistory.length > 0 ? (
                  ...
                  <p className="text-sm text-slate-500 italic">No upload history yet.</p>
                )}
              </div>
              ...
              <div className="glass-card rounded-2xl p-6">
                ...
                <div className="space-y-6 relative">
                  ...
                  <div className="flex gap-4 relative z-10">
                    <div className="w-6 h-6 rounded-full bg-blue-500 text-white flex items-center justify-center font-bold text-xs shrink-0 shadow-sm shadow-blue-200 border-2 border-white">
                      1
                    </div>
                    <div>
